...
interface UserDashboardProps {
  ...
  onCreateUser: () => void;
  onExportUserPlans: (user: User) => void;
  onRunTests?: () => void;
}

export const UserDashboard: React.FC<UserDashboardProps> = ({ users, onSelectUser, onCreateUser, onExportUserPlans, onRunTests }) => {
  return (
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        ...
        <div className="flex gap-4">
          {onRunTests && (
            <button 
              onClick={onRunTests}
              className="px-6 py-4 bg-[#F2F5F0] text-[#5B7A4B] border border-[#E5EBE0] rounded-2xl font-black uppercase tracking-widest text-[10px]"
            >
              Run Diagnostics
            </button>
          )}
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {users.length === 0 ? (
          <Card className="col-span-full py-20 bg-white border-2 border-dashed border-[#EBE8E0] flex flex-col items-center justify-center text-center px-10">
             ...
             <p className="text-[#A6A196] max-w-xs mb-8">No culinary architects detected. Initialize a profile to begin.</p>
          ...
          users.map(user => (
            ...
            >
              <div className="flex justify-between items-start">
                 <Badge variant="slate">{user.cityType} Economy</Badge>
                 {user.preferences?.highQualityVisuals && <Badge variant="terracotta">HQ</Badge>}
              </div>
              <div>
                 <h3 className="text-3xl font-black text-[#2D2A26] group-hover:text-[#E2725B] transition-colors leading-tight serif italic">{user.name}, {user.age}</h3>
                 <p className="text-[10px] font-black uppercase tracking-widest text-[#A6A196] mt-2">Daily Goal: ₹{user.dailyBudget}</p>
              </div>
              ...
              <button className="w-full py-4 bg-[#2D2A26] text-white rounded-xl font-black text-[10px] uppercase tracking-widest transition-all group-hover:bg-black">
                Manage Blueprint
              </button>
            </Card>
          ))
